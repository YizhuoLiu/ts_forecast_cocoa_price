...
data <- subset(data, Date < as.Date("2022-01-01"))
...
month_data_con$Date <- as.Date(paste("01/", year_data_con$Date, sep = "/"), format = "%d/%m/%Y")
...
```{R}
GAM_data_lag <- GAM_data_lag %>% 
  select(Date, Price, starts_with("lag_"))
